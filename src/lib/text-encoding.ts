const WINDOWS_1252_BYTES = new Map<number, number>([
  [0x20ac, 0x80], [0x201a, 0x82], [0x0192, 0x83], [0x201e, 0x84],
  [0x2026, 0x85], [0x2020, 0x86], [0x2021, 0x87], [0x02c6, 0x88],
  [0x2030, 0x89], [0x0160, 0x8a], [0x2039, 0x8b], [0x0152, 0x8c],
  [0x017d, 0x8e], [0x2018, 0x91], [0x2019, 0x92], [0x201c, 0x93],
  [0x201d, 0x94], [0x2022, 0x95], [0x2013, 0x96], [0x2014, 0x97],
  [0x02dc, 0x98], [0x2122, 0x99], [0x0161, 0x9a], [0x203a, 0x9b],
  [0x0153, 0x9c], [0x017e, 0x9e], [0x0178, 0x9f],
]);

const MOJIBAKE_MARKERS = new Set([0x00c2, 0x00c3, 0x00e2, 0x00ef, 0x00f0]);

export function repairUtf8Mojibake(value: string) {
  let candidate = value;
  for (let pass = 0; pass < 3; pass += 1) {
    const candidateScore = mojibakeScore(candidate);
    if (!candidateScore) break;
    const bytes: number[] = [];
    let convertible = true;
    for (const character of candidate) {
      const codePoint = character.codePointAt(0) ?? 0;
      const byte = codePoint <= 0xff ? codePoint : WINDOWS_1252_BYTES.get(codePoint);
      if (byte === undefined) {
        convertible = false;
        break;
      }
      bytes.push(byte);
    }
    if (!convertible) break;

    let decoded: string;
    try {
      decoded = new TextDecoder("utf-8", { fatal: true }).decode(Uint8Array.from(bytes));
    } catch {
      break;
    }
    if (mojibakeScore(decoded) >= candidateScore) break;
    candidate = decoded;
  }
  return candidate;
}

function mojibakeScore(value: string) {
  let score = 0;
  for (const character of value) {
    if (MOJIBAKE_MARKERS.has(character.codePointAt(0) ?? 0)) score += 1;
  }
  return score;
}
