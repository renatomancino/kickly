...
export function MatchCard({ match, showLeague = true }: { match: MatchSummary; showLeague?: boolean }) {
  ...
  return (
    <Link className="block" href={`/matches/${match.id}`}>
      <Card className="overflow-hidden transition-colors hover:border-primary/25 hover:bg-card/80">
        <CardContent className="flex gap-4">
          <div className="grid size-16 shrink-0 place-items-center rounded-2xl bg-primary/10 text-center text-primary">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground"><span className="flex items-center gap-1"><MapPin className="size-3.5" />{match.locationName}</span><span className="flex items-center gap-1"><UsersRound className="size-3.5" />{match.goingCount}/{match.maxPlayers}</span><span className="flex items-center gap-1"><CalendarDays className="size-3.5" />{match.footballFormat}</span></div>
