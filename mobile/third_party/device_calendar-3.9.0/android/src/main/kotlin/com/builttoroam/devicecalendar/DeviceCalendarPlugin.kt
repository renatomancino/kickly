...
// La 3.9.0 pubblicata registrava il plugin solo con la "v1 embedding"
// (`registerWith(Registrar)`), rimossa dall'engine Flutter da diverse
// versioni. Conseguenza: su Android non solo il codice non compilava più, ma
// il plugin non sarebbe comunque mai stato registrato (non compariva in
// GeneratedPluginRegistrant), e ogni chiamata dal Dart sarebbe fallita con
// MissingPluginException. Qui è portato alla "v2 embedding": FlutterPlugin per
// il ciclo di vita dell'engine, ActivityAware per avere l'Activity necessaria
// alla richiesta dei permessi calendario.
class DeviceCalendarPlugin : MethodCallHandler, FlutterPlugin, ActivityAware {
    // Methods
    ...
    // --- FlutterPlugin: ciclo di vita legato all'engine ---------------------

    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        // L'Activity non è ancora disponibile a questo punto: arriva dopo con
        // onAttachedToActivity. Qui basta l'applicationContext, che è quello
        // usato per tutte le query al ContentResolver del calendario.
        _calendarDelegate = CalendarDelegate(null, binding.applicationContext)
        _channel = MethodChannel(binding.binaryMessenger, CHANNEL_NAME).apply {
            setMethodCallHandler(this@DeviceCalendarPlugin)
        }
    }
    ...
    // --- ActivityAware: serve l'Activity per richiedere i permessi ----------

    override fun onAttachedToActivity(binding: ActivityPluginBinding) {
        _activityBinding = binding
        _calendarDelegate.setActivity(binding.activity)
        // Sostituisce registrar.addRequestPermissionsResultListener della v1:
        // senza questo, la risposta dell'utente al dialog dei permessi non
        // tornerebbe mai al delegate e la chiamata Dart resterebbe appesa.
        binding.addRequestPermissionsResultListener(_calendarDelegate)
    }

    override fun onDetachedFromActivity() {
        _activityBinding?.removeRequestPermissionsResultListener(_calendarDelegate)
        _activityBinding = null
        _calendarDelegate.setActivity(null)
    }

    // Rotazione schermo e simili: l'Activity viene distrutta e ricreata, il
    // trattamento è lo stesso dell'aggancio/sgancio normale.
