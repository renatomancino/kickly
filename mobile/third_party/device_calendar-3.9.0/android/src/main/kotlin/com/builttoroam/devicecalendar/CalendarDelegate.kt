...
// Serve per richiedere i permessi a runtime: nell'originale l'Activity veniva
// presa dal `Registrar` della v1 embedding, che non esiste più (vedi sotto).
...
class CalendarDelegate : PluginRegistry.RequestPermissionsResultListener {
    ...
    // Nell'originale qui c'era un `Registrar` (v1 embedding), usato solo per
    // ottenere context e activity. Il Registrar non esiste più nell'engine
    // Flutter, quindi il context arriva dal FlutterPlugin e l'activity viene
    // aggiornata dai callback ActivityAware (può essere null quando l'app è in
    // background: in quel caso non si possono chiedere permessi).
    ...
    // Chiamata da DeviceCalendarPlugin a ogni aggancio/sgancio dell'Activity.
    ...
    private fun arePermissionsGranted(): Boolean {
        if (atLeastAPI(23)) {
            // Il controllo dei permessi non richiede l'Activity: basta il
            // context del plugin, che è sempre disponibile.
            val writeCalendarPermissionGranted = _context!!.checkSelfPermission(Manifest.permission.WRITE_CALENDAR) == PackageManager.PERMISSION_GRANTED
            val readCalendarPermissionGranted = _context!!.checkSelfPermission(Manifest.permission.READ_CALENDAR) == PackageManager.PERMISSION_GRANTED
    ...
    private fun requestPermissions(requestCode: Int) {
        if (atLeastAPI(23)) {
            // Senza Activity agganciata non è possibile mostrare il dialog di
            // sistema: si esce senza crashare (l'originale faceva `!!` sul
            // Registrar, che qui non è più garantito non-null).
    ...
    private fun parseRecurrenceRuleString(recurrenceRuleString: String?): RecurrenceRule? {
        ...
        when (rfcRecurrenceRule.freq) {
            ...
            // Le altre frequenze (DAILY, HOURLY, MINUTELY, SECONDLY) non hanno
            // un "giorno della settimana" da estrarre. L'originale le lasciava
            // semplicemente fuori dal `when`; da Kotlin 1.7 un `when` non
            // esaustivo su enum è un errore, quindi il ramo va reso esplicito.
            else -> {}
        }
